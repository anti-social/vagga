...
#     [[ $link = ".roots/py2-git-ubuntu.ebda7dbd/root" ]]
# }
...
#     [[ $link = ".roots/py3-git-ubuntu.ce381ca9/root" ]]
# }
...
@test "pip: py3 compile" {
    rm -rf app/__pycache__
    run vagga _build py3-compile
    printf "%s\n" "${lines[@]}"
    [[ $status = 0 ]]
    [[ -d app/__pycache__ ]]
    sum=$(sha1sum app/__pycache__/test_compile.cpython-35.pyc)

    rm -rf app/__pycache__
    run vagga _build --force py3-compile
    printf "%s\n" "${lines[@]}"
    [[ $status = 0 ]]
    new_sum=$(sha1sum app/__pycache__/test_compile.cpython-35.pyc)
    echo $sum
    echo $new_sum
    [[ $new_sum = $sum ]]
